...
unsigned int static GetNextWorkRequired(const CBlockIndex* pindexLast, const CBlockHeader *pblock)
{
    int DiffMode = 1;
    if (fTestNet) {
        if (pindexLast->nHeight+1 >= 16) { DiffMode = 3; }
    }
    else {
        if (pindexLast->nHeight > HARDFORK_HEIGHT_2) { DiffMode = 3; }
        else if (pindexLast->nHeight > HARDFORK_HEIGHT_1) { DiffMode = 2; }
        //else if (pindexLast->nHeight > 15200) { DiffMode = 1; }
    }
    ...
    switch (DiffMode) {
        ...
        case 3:
            ...
            break;
        case 4:
            return GetNextWorkRequired_V4(pindexLast, pblock);
            break;
        default:
            return GetNextWorkRequired_V3(pindexLast, pblock);
    }
